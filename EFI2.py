...
def inicio():
    layout = [
                ...
                [sg.B('Jugar',button_color='#ffd424',bind_return_key=True,pad=(15,5),font=('Arial 18'))],
    ]
    return sg.Window('Inicio', layout=layout,size=(300,160),element_justification='center',grab_anywhere = True,finalize=True)

def juego():
    layout = [
                [sg.T(guiones,font=('Georgia 15'),pad=(250,0),text_color=('white'),key='-GUIONES-'),sg.T(f'Intentos: {intentos}',font=('Georgia'),text_color=('white'),key='-INTENTOS-')],
                [sg.Image(imagenes[5],pad=(250,20),key='-IMAGENES-')],
                [sg.T('Ingrese una letra:',text_color=('white'),font=('Georgia')),sg.I(text_color=('white'),size=(2,0),font=('Georgia'),key='-SALIDA-'),sg.T(size=(40,1),text_color=('white'),key='-AVISO-')],
                [sg.B('Enviar',button_color='#ffd424',bind_return_key=True,font=('Arial')),sg.B('Salir',button_color='#ffd424',font=('Arial')),sg.T(pad=(250,0),font=('Georgia'),text_color=('white'),key='-LETRA_FALLADA-')],
    ]
    return sg.Window('Juego ahorcado',layout=layout,no_titlebar=True,grab_anywhere = True,finalize=True)
